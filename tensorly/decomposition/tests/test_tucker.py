...
def test_partial_tucker():
    ...
    for svd_func in tl.SVD_FUNS:
        ...
        assert_(tl.to_numpy(tl.max(tl.abs(norm_rec - norm_tensor))) < tol_max_abs)

        # Test the shape of the core and factors
        ranks = [3, 1]
        core, factors = partial_tucker(tensor, modes=modes, rank=ranks, n_iter_max=100, svd=svd_func,
                                       verbose=1)
        for i, rank in enumerate(ranks):
            assert_equal(factors[i].shape, (tensor.shape[i+1], ranks[i]),
                         err_msg="factors[{}].shape={}, expected {} (svd=\"{}\")".format(
                             i, factors[i].shape, (tensor.shape[i+1], ranks[i]), svd_func))
        assert_equal(core.shape, [tensor.shape[0]]+ranks, err_msg="Core.shape={}, "
                         "expected {} (svd=\"{}\")".format(core.shape, [tensor.shape[0]]+ranks, svd_func))
...
def test_tucker():
    ...
    tensor = tl.tensor(rng.random_sample((3, 4, 3)))
    for svd_func in tl.SVD_FUNS:
        core, factors = tucker(tensor, rank=None, n_iter_max=200, svd=svd_func, verbose=True)
        ...
        # Test the max abs difference between the reconstruction and the tensor
        assert(tl.to_numpy(tl.max(tl.abs(reconstructed_tensor - tensor))) < tol_max_abs)

        # Test the shape of the core and factors
        ranks = [2, 3, 1]
        core, factors = tucker(tensor, rank=ranks, n_iter_max=100, svd=svd_func, verbose=1)
        for i, rank in enumerate(ranks):
            assert_equal(factors[i].shape, (tensor.shape[i], ranks[i]),
                         err_msg="factors[{}].shape={}, expected {} (svd=\"{}\")".format(
                             i, factors[i].shape, (tensor.shape[i], ranks[i]), svd_func))
            assert_equal(tl.shape(core)[i], rank, err_msg="Core.shape[{}]={}, "
                         "expected {} (svd=\"{}\")".format(i, core.shape[i], rank, svd_func))

        # Random and SVD init should converge to a similar solution
        tol_norm_2 = 10e-1
        tol_max_abs = 10e-1

        core_svd, factors_svd = tucker(tensor, rank=[3, 4, 3], n_iter_max=200, init='svd', svd=svd_func, verbose=1)
        core_random, factors_random = tucker(tensor, rank=[3, 4, 3], n_iter_max=200, init='random', svd=svd_func,
                                             random_state=1234)
        ...
        assert_(tl.to_numpy(error) < tol_norm_2,
                'norm 2 of difference between svd and random init too high '
                '(svd="{0}")'.format(svd_func))
        assert_(tl.to_numpy(tl.max(tl.abs(rec_svd - rec_random))) < tol_max_abs,
